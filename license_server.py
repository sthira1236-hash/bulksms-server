...
def verify():
    ...
    if not saved_device:
        ...
        conn.commit()
        conn.close()
        return jsonify({"status": "valid", "message": "Device registered"})
    ...
    # DIFFERENT DEVICE
    conn.close()
    return jsonify({"status": "blocked", "message": "Used on another device"})
...
def dashboard():
    ...
    for d in data:
        ...
        <tr>
            ...
            <td>{d[2]}</td>
            <td>{d[3]}</td>
            <td><a href="/reset_device/{d[0]}">Reset</a></td>
        </tr>
        """
